...
PS.views.settings = () => {
  ...
  const html = `<div class="page">${mobileBar()}<div class="eyebrow">Instellingen</div><h1 class="display">Jouw studio.</h1>
    <div class="card card-pad" style="margin-top:18px">
      ...
      <div class="setting"><div><div class="t">Schrijven met Apple Pencil</div><div class="s">Grote schrijfvakken. Nederlands, Frans en Engels zet je iPad om naar tekst (iPad-instellingen → Apple Pencil → Scribble), dus die worden gewoon nagekeken. Grieks leest je iPad niet: bij het oefenen schrijf je in een schrijfvak en vergelijk je zelf; in toetsen typ je het.${PS.pen.seen() ? '' : ' Gaat vanzelf aan zodra je de Pencil gebruikt.'}</div></div><div class="seg" data-pen-seg>${[['auto', 'Auto'], ['on', 'Aan'], ['off', 'Uit']].map(([v, l]) => `<button type="button" data-v="${v}" aria-pressed="${(st.pen || 'auto') === v}">${l}</button>`).join('')}</div></div>
  ...
  const after = (root) => {
    ...
    PS.$('[data-pen-seg]', root).addEventListener('click', (e) => { const b = e.target.closest('[data-v]'); if (!b) return; PS.S.s.settings.pen = b.dataset.v; PS.S.save(); PS.pen.apply(); PS.render(); });
